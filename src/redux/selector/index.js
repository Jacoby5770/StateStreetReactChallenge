export const getIndividualTransaction = (data, accountNumber) => {
...
export const filterAccountSelecter = (data, accountType, transactionType) => {
    ...
    return data['transactions'].reduce((result, id) => {
      accountType.forEach((account) => {
        transactionType.forEach((transaction) => {
          if (account===id['accountName'] && transaction===id['transactionType']) {
            result.push(id)
            return result
          }
        })
